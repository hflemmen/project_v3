...
func ShouldStop(e Elevator) bool {
	switch e.Dir {
	case MD_Down:
		return e.Order[BT_HallDown][e.Floor] != 0 ||
		       e.Order[BT_Cab][e.Floor] != 0 ||
		       !orderBelow(e)
	case MD_Up:
		return e.Order[BT_HallUp][e.Floor] != 0 ||
		       e.Order[BT_Cab][e.Floor] != 0 ||
		       !orderAbove(e)
	case MD_Stop:
		fallthrough
...
func ClearLightsAtCurrentFloor(e Elevator ) Elevator {
	e2 := e.Duplicate()
	for btn := 0; btn < 2; btn++ {
		if e2.LightMatrix[btn][e2.Floor] != 0 {
			e2.LightMatrix[btn][e2.Floor] = 0
		}
	}
